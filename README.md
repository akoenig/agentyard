...
- `agents/research/compose.yaml`
- `agents/research/caddy.caddy`
- `agents/research/litellm-config.yaml`
...
```bash
docker compose -f compose.yaml -f agents/research/compose.yaml up -d caddy litellm-research
docker compose -f compose.yaml -f agents/research/compose.yaml logs -f litellm-research
```
...
```bash
docker compose -f compose.yaml -f agents/research/compose.yaml up -d hermes-research caddy
```

Run Hermes setup for that agent if needed:

```bash
docker compose -f compose.yaml -f agents/research/compose.yaml run --rm hermes-research setup
```
...
docker compose \
  -f compose.yaml \
  -f agents/research/compose.yaml \
  -f agents/work/compose.yaml \
  up -d
```
...
```caddyfile
import /etc/caddy/agents/*/caddy.caddy
```

The root Compose file mounts `./agents` into Caddy at `/etc/caddy/agents`, so each `agents/<name>/caddy.caddy` file is picked up by the import.

The repository includes `agents/_placeholder/caddy.caddy` so Caddy has a valid import target before the first real agent is created. Directories beginning with `_` are internal and are ignored by `scripts/list-agents`.
